...
conn ipke2vpn
    ...
    leftid="C=CN, O=huayu, CN=$public_ip"
    right=%any
...
function set_up_php7(){
	rpm -Uvh https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm
    rpm -Uvh https://mirror.webtatic.com/yum/el7/webtatic-release.rpm
    yum -y install php70w php70w-opcache
	yum -y install php70w-common
	yum -y install php70w-fpm
	yum -y install php70w-pdo
...
#获取实际的strongswanpsk
strongswanPSK=`cat /usr/local/etc/ipsec.secrets | grep PSK |awk -F '"' {'print $2'}`
...
if [ $changePubIP == "y" ];then
	...
	sed -i "s/$currPubIP/$newPubIP/g" /usr/local/etc/ipsec.conf 
